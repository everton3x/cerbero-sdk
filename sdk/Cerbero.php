...
final class Cerbero
{
    ...
    {
        if (!$this->authenticated($userId, $sessionToken)) {
            throw new UserNotAuthenticated($userId);
        }
        if (is_null($this->pdo)) {
            throw new RuntimeException('No database connection');
        }
    ...
    {
        if (!$this->access($userId, $sessionToken, $systemSlug)) {
            throw new UserNotAuthorized($userId, $systemSlug);
        }
        if (is_null($this->pdo)) {
            throw new RuntimeException('No database connection');
        }
    ...
    public function authenticate(string $userId, string $password): string
    {
        ...
        if (isset($this->config['maxLoginAttempts'])) {
            ...
            if ($attempts > $this->config['maxLoginAttempts']) {
                throw new LimitLoginAttempts($userId);
            }
        }
        ...
        if (!is_array($result) || !isset($result['password_hash']) || !is_string($result['password_hash'])) {
            throw new UserOrPasswordInvalid($userId);
        }
